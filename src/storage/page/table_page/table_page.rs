...
impl TablePage {
    ...
    fn new(page_id: PageId, next_page_id: PageId) -> TablePage {
        TablePage {
            page_id,
            next_page_id, // TODO: either actually use this field or remove it.
...
impl Page for TablePage {
    type InsertOutputType = SlotId;
    type ConcretePageType = Self;

    /// Fetch a tuple from the table page using the `RecordId`.
    ///
    /// If the `RecordId` is invalid to be used on this page, the function returns an `InvalidInput`
    /// Error. Otherwise, it fetches the tuple from the content of the page.
    ///
    /// # Parameters
    /// - `RecordId`: The identifier of a record instance
    ///
    /// # Returns
    ...
    ///
    /// If the page does not have enough room to insert the next tuple, it shall return `None.`
    /// Otherwise, it inserts the tuple within the page and updates the relevant meta information
    /// of the page.
    ///
    /// # Parameters
    /// - `TupleMetadata`: The identifier of a record instance
    /// - `Tuple`: The Tuple Instance
    ///
    /// # Returns
    /// - `Some(SlotID)`: the `SlotID` of the tuple
    /// - `None`: If the tuple cannot be inserted into the page
    fn insert_tuple(
        &mut self,
        meta: TupleMetadata,
        tuple: Tuple,
    ...
    ///
    /// If the `RecordId` is valid to be fetched on this page, returns an `InvalidInput` Error.
    /// Otherwise, fetch the corresponding metadata of the tuple using the `RecordId`.
    ///
    ///
    /// # Parameters
    /// - `RecordId`: The identifier of a record instance
    ...
    ///
    /// If the `RecordId` is valid to be fetched on this page, returns an `InvalidInput` Error.
    /// Otherwise, fetch the corresponding metadata of the tuple using the `RecordId`, and updates
    /// its metadata.
    ///
    /// # Parameters
    /// - `TupleMetadata`: The new metadata of the tuple
    /// - `RecordId`: The identifier of a record instance
    ///
    /// # Returns
    /// - `Error`: If the `RecordID` is invalid to be used on this page
    ...
    ///
    /// # Parameters
    /// - `is_dirty`: New dirty state to set
    ///
    /// # Returns
...
impl Clone for TablePageIterator {
    fn clone(&self) -> Self {
        let page = Arc::clone(&self.page);
        Self {
            page,
            index: AtomicU16::new(self.index.load(Ordering::SeqCst)),
        }
    }
}
